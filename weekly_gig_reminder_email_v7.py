...
def read_comment_via_html_editor() -> str:
    """
    Launch LibreOffice Writer (or another WYSIWYG HTML editor) to compose
    rich-text comments. After saving and closing, extracts only the <body>
    content as HTML suitable for embedding in the email.
    """
    editor = os.environ.get("EDITOR", "libreoffice")
    with tempfile.NamedTemporaryFile(suffix=".html", delete=False) as tf:
        temp_path = tf.name

    # Create a minimal starter HTML file
    html_template = """<!DOCTYPE html>
<html>
<head><meta charset="utf-8"><title>Comments</title></head>
<body>
<p><em>Type your comments here.</em></p>
</body>
</html>"""
    Path(temp_path).write_text(html_template, encoding="utf-8")

    print(f"\nOpening {editor} to compose HTML comments...")
    try:
        subprocess.call([editor, temp_path])
    except FileNotFoundError:
        print(f"⚠️  Editor '{editor}' not found. Falling back to xed.")
        subprocess.call(["xed", temp_path])

    # Read file contents
    html_content = Path(temp_path).read_text(encoding="utf-8")

    # Extract <body>...</body>
    match = re.search(r"<body[^>]*>(.*?)</body>", html_content, flags=re.I | re.S)
    if match:
...
def main():
    ...
    # ← dedent this section so it always runs
    USE_HTML_EDITOR = True  # toggle True for LibreOffice, False for xed

    if USE_HTML_EDITOR:
        comment_html = read_comment_via_html_editor()
    else:
        print("\nOpening plain-text editor to enter comments...")
