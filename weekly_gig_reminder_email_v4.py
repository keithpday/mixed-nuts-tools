...
# -*- coding: utf-8 -*-
"""
weekly_gig_reminder_email_v4.py
───────────────────────────────────────────────────────────────────────────────
...
Key behavior:
  ...
  2) Prompts for comment paragraphs (each entered separately)
...
# ──────────────────────────────────────────────────────────────────────────────
# Comment input helper — simplified for multi-paragraph support
# ──────────────────────────────────────────────────────────────────────────────

def read_comment_paragraphs() -> str:
    """
    Prompt for one or more comment paragraphs.
    Each comment entered becomes its own <p> paragraph in the email.
    Returns an HTML string (0+ <p> blocks).
    """
    paragraphs = []
    print("\nEnter any comments to include before sign-off.")
    
    first = input("Enter first comment paragraph (or press Enter for none): ").strip()
    if not first:
        return ""  # no comment
    paragraphs.append(first)

    while True:
        ans = input("Add another comment? (y/n): ").strip().lower()
        if ans == "y":
            next_comment = input("Enter next comment paragraph: ").strip()
            if next_comment:
                paragraphs.append(next_comment)
        elif ans in ("n", ""):
            break
        else:
            print("Please enter 'y' or 'n'.")

    # Join paragraphs as HTML <p> blocks
...
def main():
    ...
    # New: simple multi-paragraph comment flow
