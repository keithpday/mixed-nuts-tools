...
# -*- coding: utf-8 -*-
"""
weekly_gig_reminder_email_v6.py
───────────────────────────────────────────────────────────────────────────────
...
Key behavior:
  1) Prompts for week-begin date (default = next Sunday) and validates Sunday
  2) Prompts for comments: type/paste multi-paragraph text; finish with ENTER twice
...
def _first_token(cell: str) -> str:
    ...
    if not s:
        return ""
    s = re.sub(r"\s*\([^)]*\)\s*$", "", s)  # remove trailing parens like " (sub)"
...
# ──────────────────────────────────────────────────────────────────────────────
# Paste-friendly multi-paragraph comment capture (finish with ENTER twice)
# ──────────────────────────────────────────────────────────────────────────────

def read_comment_paragraphs() -> str:
    """
    Capture comments from the terminal, allowing typed OR pasted multi-paragraph text.
    End input by pressing ENTER twice (i.e., a blank line after a blank line).
    Paragraphs are separated by one or more blank lines. Line breaks inside a
    paragraph are preserved with <br>. Returns an HTML string with 0+ <p> blocks.
    """
    print("\nEnter any comments you'd like included before the sign-off.")
    print("You can type or paste multiple paragraphs.")
    print("Entering two blank lines ends comment entry.\n")
    ...
    while True:
        try:
            ...
            break  # support redirected input if ever used
        if line.strip() == "":
            blank_streak += 1
            lines.append("")  # keep blanks so paragraph splitting is accurate
            if blank_streak >= 2:
                break
            continue
        else:
    ...
    return "".join(f"<p>{escape(p).replace('\n', '<br>')}</p>" for p in paragraphs)
...
def normalize_signoff(body_html: str) -> str:
    s = body_html
    # Remove existing Best/— Keith blocks (avoid duplicates)
...
def inject_comment_before_signoff(body_html: str, comment_html: str) -> str:
    """Insert comment_html (0+ <p> blocks) just before the signoff."""
    comment_html = (comment_html or "").strip()
    if not comment_html:
        return body_html
    patterns = [
    ...
    return body_html + comment_html  # fallback if not found
...
FALLBACK_TOKEN_STYLE = {
    ...
    "none": {"bg":"#000000", "fg":"#ffffff", "bold":False, "italic":False, "underline":False},
}
...
def load_band_members(values):
    ...
    if DEBUG:
        print(f"\n[DEBUG] Loaded {len(key_to_style)} styled names from BandMembers:")
        for k, v in key_to_style.items():
            print(f"   {k:20} → {v}")
        print(f"[DEBUG] Token styles: {token_style}\n")
...
def style_for(cell_text: str, key_to_style: dict, token_style: dict) -> str:
    ...
    # Debug logging
    debug_print(f"style_for: cell_text='{cell_text}' → token='{low}'")

    if low in {"yes","no","sub","none"}:
        spec = token_style.get(low) or FALLBACK_TOKEN_STYLE.get(low, {})
        debug_print(f"  → Using token_style or fallback for '{low}': {spec}")
        return style_css_from_spec(spec)

    spec = key_to_style.get(_canon(tok), {})
    debug_print(f"  → Using key_to_style for '{tok}': {spec}")
    return style_css_from_spec(spec)
...
def stack_time_cell(s: str) -> str:
    if not s: return ""
    # Split first dash into a line break: "1:00 – 2:00" -> "1:00 –<br>2:00"
...
def read_week_rows(creds, start: date, end: date):
    ...
    idx_vocal, idx_piano, idx_bass, idx_drums, idx_guitar, idx_vibes = (
        idx("vocal(s)?"), idx("(piano|keys?|keyboard)"), idx("bass"),
        idx("drum(s)?"), idx("guitar"), idx("(vibes?|vibraphone)")
    )
    ...
    for r in rows:
        ...
        if start <= d <= end:
            week_rows.append({
                ...
                "venue": val(r, idx_venue),
                "time": val(r, idx_time),
                "location": val(r, idx_loc),
                "set": val(r, idx_set),
                "pays": val(r, idx_pays),
                "vocal": val(r, idx_vocal),
                "piano": val(r, idx_piano),
                "bass": val(r, idx_bass),
                "drums": val(r, idx_drums),
                "guitar": val(r, idx_guitar),
                "vibes": val(r, idx_vibes),
            })
    return week_rows, {}

def build_email_html(week_rows, members_blob, start: date, end: date, comment_html: str):
    ...
    if DEBUG:
        debug_print(f"[DEBUG] Building email for {len(week_rows)} gigs, week {start}–{end}")

    # ensure canonical signoff, then inject comments just before it
    body_html = normalize_signoff(body_html)
    body_html = inject_comment_before_signoff(body_html, comment_html)
...
def read_comment_via_editor() -> str:
    """
    Open the user's preferred text editor (xed by default) to compose
    the comment(s). Allows full multi-paragraph text and formatting freedom.
    Returns HTML with <p> and <br> tags for paragraphs and line breaks.
    """
    editor = os.environ.get("EDITOR", "xed")
    with tempfile.NamedTemporaryFile(suffix=".txt", delete=False) as tf:
        temp_path = tf.name

    # Preload helpful guidance
    with open(temp_path, "w", encoding="utf-8") as f:
        f.write("# Enter your comments below.\n")
        f.write("# You can type or paste multiple paragraphs.\n")
        f.write("# Blank lines are fine — each paragraph becomes its own block.\n")
        f.write("# Lines beginning with '#' are ignored.\n")
        f.write("# When finished, save and close this window.\n")
        f.write("# ------------------------------------------------------------\n")
        f.write("# Inline HTML markup examples (these work inside sentences):\n")
        f.write("#   <a href=\"https://example.com\">Clickable link</a>\n")
        f.write("#   <b>Bold text</b> or <i>Italic text</i>\n")
        f.write("#   <span style=\"color:blue;\">Colored words</span>\n")
        f.write("#   Add <br> if you really want a line break inside a paragraph.\n")
        f.write("# ------------------------------------------------------------\n\n")
    ...
    try:
        ...
        print(f"⚠️  Editor '{editor}' not found. Falling back to nano.")
...
def main():
    ...
    if args.week_of:
        ...
        while True:
            ...
            try:
                ...
                continue
            if base.weekday() != 6:  # Sunday = 6
                print("⚠️  The date you entered is not a Sunday. Please enter a Sunday date.")
                continue
            break
        week_start, week_end = week_range(base)

    # Compose comments via external editor (xed or $EDITOR)
    print("\nOpening editor to enter comments...")
    comment_html = read_comment_via_editor()


    # Inform the user we're fetching data and building the email
    print("\nNow generating email, please wait...")
    ...
    subject, body_html = build_email_html(week_rows, members_blob, week_start, week_end, comment_html)
