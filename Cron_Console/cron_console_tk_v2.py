...
class JobDialog(tk.Toplevel):
    def __init__(self, master, title, init=None):
        ...
        ttk.Entry(frm, textvariable=self.var_desc, width=80).grid(row=6, column=0, sticky="we")
...
class App(tk.Tk):
    def __init__(self):
        ...
        ttk.Button(top, text="View crontab -l", command=self.view_crontab_raw).pack(side="left", padx=6)
    ...
    def refresh_table(self):
        ...
        # External rows first or after? Let’s show CCM first for emphasis.
    ...
    # ----- Raw viewer -----
    def view_crontab_raw(self):
        rc, out, err = run(["crontab", "-l"])
        text = out if rc == 0 else "(no crontab for user)\n"
        win = tk.Toplevel(self)
        win.title("Current User Crontab (View Only)")
        # Big but slightly smaller than main window
        win.geometry("2800x1300")

        top = ttk.Frame(win)
        top.pack(fill="x", padx=8, pady=(8,0))
        ttk.Button(top, text="Copy to Clipboard", command=lambda: self._copy_text(text)).pack(side="left")
        ttk.Label(top, text="Output of: crontab -l").pack(side="left", padx=12)
